...
#if MEMSTREAM_DEBUG
static void
memstream_print(struct memstream *ms)
{
...
static int
memstream_grow(struct memstream *ms, int minsize)
{
...
static int
memstream_read(void *cookie, char *buf, int count)
{
...
static int
memstream_write(void *cookie, const char *buf, int count)
{
	struct memstream *ms = (struct memstream *)cookie;
	memstream_check(ms);
...
static fpos_t
memstream_seek(void *cookie, fpos_t offset, int whence)
{
	struct memstream *ms = (struct memstream *)cookie;
	fpos_t pos = 0;
	memstream_check(ms);
